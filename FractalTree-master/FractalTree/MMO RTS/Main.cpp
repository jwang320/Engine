...
static SpaceGameClient::SpaceGameApplication application;
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE, LPSTR strCmdLine, int)
{
	...
	windowWidth = min(1920, winWidth);
	windowHeight = min(1080, winHeight);
	CreateMainWindow(false, windowWidth, windowHeight);
#else
	windowWidth = 1920;
	windowHeight = 1080;
	CreateMainWindow(false, 1920, 1080);
#endif
	...
	//program now initialized from spacebar.
...
void HandleInput()
{
	...
	{
		const bool shiftDown = (keystate[DIK_LSHIFT] & 0x80) || (keystate[DIK_RSHIFT] & 0x80);
		//Alphabet keys
		for (int a = 1; a < 60; ++a)
		{
			...
			{
				keydown[a] = true;
				application.KeyPressed(asciivalues[a], shiftDown);
			}
